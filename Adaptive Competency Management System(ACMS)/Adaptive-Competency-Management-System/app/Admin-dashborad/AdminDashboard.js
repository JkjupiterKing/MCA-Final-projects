    ...
    function fetchDetails(apiUrl, type) {
        fetch(apiUrl)
                        ...
                        data.forEach(enrollment => {
                            if (enrollment.status === 'Completed') {
                                detailsHtml += `
                                    <li class="list-group-item">
                                        <h5>${enrollment.course.courseName}</h5>
                                        <p>Completion Date: ${new Date(enrollment.enrollmentDate).toLocaleDateString()}</p>
                                    </li>
                                `;
                            }
                        });
                        ...
                        data.forEach(result => {
                            detailsHtml += `
                                <li class="list-group-item">
                                    ...
                                    <p>Course: ${result.enrollment.course.courseName}</p>
                                    <p>Score: ${result.score}</p>
                                    <p>Date: ${new Date(result.assessmentDate).toLocaleDateString()}</p>
                                </li>
                            `;
                        });
                        detailsHtml += `</ul>`;
            ...
            .catch(error => {
                ...
                document.getElementById('results').innerHTML = `<p>Error fetching details. Please try again later.</p>`;
            });
    }
