...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateEmployeeSelect() {
        ...
        selectElementUpdate.innerHTML = '<option value="">Select an employee</option>';
    ...
    function displayProjects(projects) {
        ...
        pageProjects.forEach(function(project) {
            ...
            var row = '<tr data-project-id="' + projectId + '">' +
                ...
                '<td>' + (project.user ? project.user.username : 'Unknown') + '</td>' +
                '<td>' +
    ...
    function updateProject(projectData) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                alert("Error updating project. Server returned error status: " + xhr.status);
            }
        };
        xhr.onerror = function() {
            console.error('Error updating project. Network error.');
            alert("Failed to update project. Network error. Check console for details.");
        };
        xhr.send(JSON.stringify(projectData));
    }
    ...
    function openUpdateForm(projectId) {
        fetchProjectDetails(projectId, function(project) {
            ...
            document.getElementById('updateAssignedEmployee').value = project.user ? project.user.username : '';
